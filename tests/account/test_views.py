...
def test_UpdatePassword():
    ...
    """
    In the first assert, we are testing if there
    is no issue rendering template by checking 200 status code,
    For the second assert, we are checking if our view
    is returning 'change_password.html' template
    """
...
def test_SignUpView():
    ...
    then, the user is redirected to '/login/' route,
...
def test_ProfileView():
    ...
    For the second assert, we are making sure that 'profile.html'
...
def test_Update_Profile_View():
    ...
    """
    Checking if our 'UpdateProfileView' returns the 'profile.html'
      template to show update form,
    for the second assert, we are making sure everything went fine
      by checking 200 status code
    """
